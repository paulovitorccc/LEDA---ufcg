...
# jtst - uma evolcao do tst desenvolvida por Joao Brunet. 
# O tst foi desenvolvido por Dalton Serey.
# Ambos sao professores em Computacao @ UFCG.

# (c) 2011-2014 Dalton Serey, UFCG
# jtst - (c) 2016-2017 Joao Brunet, UFCG
...
class TestRunner:
    ...
    def run_iotest(self, timeout=TIMEOUT_DEFAULT):
        ...
        # Dealing with Java.
        if is_java_file(self.testee.filename):
            if self.testee.class_name != 'compilation_error':
                cmd_str = "%s -Dfile.encoding=UTF-8 %s" % (JAVA, self.testee.class_name)
            else:
                self.result['status'] = 'SyntaxError'
                # compilation error: no need to execute test
                return
        else:
...
class Testee:
    ...
    def __init__(self, filename, class_name = None):
...
#TODO move from here
def get_java_class_name(filename):
    ...
    for i in range(len(lines)):
        ...
        if index != -1:
            remaining = line[index + 5:]
            if remaining == '':
                return lines[i+1].strip().split('{')[0]
            else:
                return remaining.strip().split('{')[0]

def compile_java_file(filename, timeout=TIMEOUT_DEFAULT):
    result = {}

    cmd_str = "%s -encoding utf-8 -nowarn %s" % (JAVAC, filename)
    command = shlex.split(cmd_str.encode('utf-8'))
    ...
    try:
        ...
        # external error... must run again!
        process.terminate()
        result['status'] = 'SO Error'
        return result

    
    if process.returncode == 0:
        result['status'] = (True, get_java_class_name(filename))
    else:
        result['status'] = 's'
...
def run_tests(testees, testcases, timeout):
    ...
    for testee in testees:
        ...
        # compile testee is a java file
        if is_java_file(testee.filename):
                result = compile_java_file(testee.filename)
                if isinstance(result['status'], tuple):
                    testee.class_name = result['status'][1]
                else:
                    testee.class_name = 'compilation_error'
        ...
        # clean .class files.
        if is_java_file(testee.filename) and testee.class_name != 'compilation_error':
            try:
                for i in glob.glob('*.class'):
                    os.remove(i)
            except OSError:
                # not much to do
                print('Couldn\'t remove tmp class file.')
...
def read_cli():
    ...
    if len(args.filename) == 1 and os.path.exists(args.filename[0]):
        ...
        fn_pattern = "*%s*.java" % args.filename[0]
        filenames += glob.glob(fn_pattern)
...
def main():
    ...
    if len(sys.argv) == 3 and sys.argv[1] == '-s':
        sys.argv.pop(1)
        tstjson = read_json('tests.json')
    else:
        # read tstjson
        tstjson = read_json('tst.json')
