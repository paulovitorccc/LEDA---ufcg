...
import questoes.arvorepv.RBNode.Colour;
...
public class RBTreeImpl<T extends Comparable<T>> extends BSTImpl<T> implements RBTree<T> {
	...
	public int qtdNodeRed() {
		return qtdNodeRed((BSTNode<T>) root);
	}

	private int qtdNodeRed(BSTNode<T> node) {
		int soma = 0;
		if (node.isEmpty()) {
			return 0;
		} else {
			...
			soma += qtdNodeRed((BSTNode<T>) node.getLeft());
			soma += qtdNodeRed((BSTNode<T>) node.getRight());
		}
		return soma;
	}
}
