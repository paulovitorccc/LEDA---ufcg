 ...
 *
 */
public class VetorParametrizado<T extends Comparable<T>> {
	...
	public VetorParametrizado(int tamanho) {
		super();
	...
	public T procurar(T o) {
		...
		boolean achou = false;
		int j = 0;
		while (j <= indice && !achou) {
			if (arrayInterno[j].equals(o)) {
				result = arrayInterno[j];
				achou = true;
			}
			j++;
		}
	...
	public T minimo() {
		...
		if (!isVazio()) {
			...
			for (int j = 0; j <= indice; j++) {
				if (comparadorMinimo.compare(result, arrayInterno[j]) < 0) {
					result = arrayInterno[j];
				}
			}
		}
